...
import { Loader2, Wand2, Percent, Scale, TrendingUp, BrainCircuit, UserCheck, Check } from 'lucide-react';
...
type PredictiveAnalysisViewProps = {
    caseInfo: {
        court: string;
        caseNumber: string;
    },
...
export function PredictiveAnalysisView({ caseInfo, caseSummary }: PredictiveAnalysisViewProps) {
    const [courtLocation, setCourtLocation] = useState(caseInfo.court);
    ...
    const handleAnalyze = async () => {
        if (!courtLocation || !legalArea || !coreArgument) {
            toast({
                variant: 'destructive',
                title: 'Eingabe unvollständig',
                description: 'Bitte füllen Sie Gerichtsort, Rechtsgebiet und Kernargument aus.',
            });
            return;
        }
                ...
                <CardContent className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-2">
                            ...
                            <Input id="court-location" value={courtLocation} onChange={(e) => setCourtLocation(e.target.value)} disabled={isBusy} />
                        </div>
                        <div className="space-y-2">
                    ...
                    <div className="flex justify-end">
                        <Button onClick={handleAnalyze} disabled={isBusy || !courtLocation || !legalArea || !coreArgument}>
                            {isAnalyzing ? (
